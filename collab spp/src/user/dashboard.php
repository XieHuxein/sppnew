        ...
        <section class="about" id="Dashboard">
                    ...
                    <tbody>
                        ...
                        <td>$kolom[status]</td>";
                                ...
                                if ($kolom['status'] == "Belum bayar") {
                                    echo "
                        <td><a href='dashboard.php?idpembayaran=$kolom[id_pembayaran]&status=$kolom[status]'><button class='cta-daf-tab btn-bayar' data-idpembayaran='$kolom[id_pembayaran]' data-status='$kolom[status]'>Bayar</button></a></td>";
                                } else {
            ...
            <div class="popup-form" id="payment-form" style="display: none;">
                <div class="pay-card">
                    ...
                    $data2 = mysqli_num_rows($data1);
                    $status = isset($_GET['status']) ? $_GET['status'] : ''; // Memeriksa apakah 'status' ada dalam URL
                    $idpembayaran = isset($_GET['id_pembayaran']) ? $_GET['id_pembayaran'] : ''; // Memeriksa apakah 'id_pembayaran' ada dalam URL
                    $databayar = mysqli_query($koneksi, "SELECT * FROM pembayaran 
                        INNER JOIN siswa ON pembayaran.nisn = siswa.nisn
                        INNER JOIN kelas ON siswa.id_kelas = kelas.id_kelas 
                        WHERE id_pembayaran='$idpembayaran'");

                    while ($kolom = mysqli_fetch_array($databayar)) {
                    ?>
                    <h2>Form Pembayaran</h2>
                    <label>ID Pembayaran:</label>
                    <span id="payment-id" value="<?php echo $kolom[2]; ?>"></span>
                    <label>Status:</label>
                    <span id="payment-status"></span>
                        <h1>AKU JAWIRR NIGGA</h1>
                        <div class="db"><?php include "../system/koneksi.php"; ?> </div>
